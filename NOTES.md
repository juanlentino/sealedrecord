...
Ideas for what the command line could do, and what it will not. None of it is scheduled or promised; an idea leaves this file when it ships, as a 0.x minor, or when it is answered as a not-fit. The rules do not move for any of them: no runtime dependencies, WebCrypto only, no network, nothing saved, no model.
