...
Ideas for what the command line could do, and what it will not. None of it is scheduled or promised, and none of it is reserved: anyone may pick an idea here, build it, and open a pull request, or fork the repository and take it elsewhere. An idea leaves this file when it ships, as a 0.x minor, or when it is answered as a not-fit. See CONTRIBUTING.md for what a contribution needs. The rules do not move for any of them: no runtime dependencies, WebCrypto only, no network, nothing saved, no model.
