...
### Changed
- COMPARISON.md: CAWG Identity Assertion citations name 1.3 (DIF ratified 17 August 2026) and link the versioned URL, since https://cawg.io/identity/ still redirects to 1.2. §5.1.1 and §6.3 keep their numbers in 1.3, and §6.3 is unchanged from 1.2. **What an action's time is worth, corrected.** The claim that an action's time is the claim generator's statement and not the actor's signature was wrong. An action may carry the time it took place (C2PA 2.4 §18.15), and a contributor's identity assertion may reference the actions assertion and so sign over it (CAWG 1.3 §6.1). Every such signature is made once, after rendering, against the hard binding (CAWG 1.3 §5.1.1, §6.1), the order of actions in the array does not imply the order performed, with the only exception constraining the first element (C2PA 2.4 §18.15.1, §18.15.2), and §18.15.4.3 calls the time a simple non-trusted time-stamp. The conclusion is unchanged: neither specification seals sequence as it happens. The sentence carrying that conclusion no longer says nothing has since been reordered, inserted, or removed, which a hard binding over a signed claim does show; it names the distinction that actually holds, whether an order was sealed step by step as the work happened or written down once at the end. The paragraph was then tightened for plainness: the antecedent of what binds the actor is named rather than left as "it", the signature sentence leads with the fact instead of the abstraction, and the closing sentence says what a sealed record does and why rather than pointing back at the sentence before it. **The closing claim narrowed to what the chain proves.** It had said the order is committed as the work happens, which credits the chain with a time property it does not carry: a chain can be built entirely at the end and still hold. What the chain proves is order and integrity, that nothing can be reordered or inserted without breaking it at that entry. Whether the entries were sealed as the work happened rests on the time receipts, which are the word of whoever holds the attestation key (FORMAT.md §7), or on timestamp proofs the record carries and this reader does not check.
