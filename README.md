...
A second reader, in Go and written from the document alone, lives in `conformance/go/` and runs against the same files in CI; every place it had to guess became a sentence in FORMAT.md. CI also runs a differential fuzz: seeded mutations of the vector through both readers, failing on any disagreement in outcome or any exception (`FUZZ_N` raises the count; 10,000 pass). Both readers share an author, so together with the fuzz they test whether the document determines behaviour, not whether a stranger reads it the same way. No third-party implementation exists; one can run against the same files. `npm run vectors` regenerates the set with fresh keys; the generator verifies its own output and refuses to write a record that does not hold.
