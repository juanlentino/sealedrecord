...
A manifest speaks for one asset at the moment it was signed. Earlier contributors appear as ingredients pointing backward, each with its own manifest, and the CAWG identity assertion (section 2 below) lets several named actors sign parts of one manifest. C2PA can also place a contribution within the asset: an assertion may carry a region of interest with a temporal range, in play time from the start of the media or in wall-clock time (C2PA 2.4 §18.2, §18.3.8), and each action in an actions assertion may carry its own (§18.15.4.6). An identity assertion that references such an assertion binds the named actor to it, region included (CAWG 1.3 §5.1.1). Several actors, several ranges of one recording, each signed by the person named: that composition is specified and it works.

What that composition binds is the actor to a span of the finished asset. An action may carry the time it took place (C2PA 2.4 §18.15), and a contributor's identity assertion may reference the actions assertion and so sign over it (CAWG 1.3 §6.1). The signature is still made once, after the asset is rendered, because every identity assertion must reference the hard binding over the final bytes (CAWG 1.3 §5.1.1, §6.1). The order of actions in the array does not imply the order in which they were performed, and the only exception constrains the first element (C2PA 2.4 §18.15.1, §18.15.2). The specification calls the time an action carries a simple non-trusted time-stamp (§18.15.4.3). Neither specification seals sequence as it happens. Nothing in C2PA or CAWG can show that an order was sealed step by step as three people worked over a given hour, rather than written down once at the end. A sealed record commits the order itself: each entry is signed by its actor and chained to the one before it, so nothing can be reordered or inserted without breaking the chain at that entry. Whether the entries were sealed as the work happened rests on the time receipts, which are the word of whoever holds the attestation key (FORMAT.md §7), or on timestamp proofs the record can carry and this reader does not check. Change entry 4 and the chain breaks at entry 4, and the reader reports both digests.
...
The record's entries can also be rendered into the manifest's own vocabulary. Each entry that anchors audio becomes an action with a temporal region on the finished asset, and each contributor's identity assertion references the actions that are theirs. The manifest then shows, in a form validators already read, who is bound to which span. The record remains the source: it holds the order those entries were sealed in and the signatures made at the time, which the manifest cannot carry and does not claim to.
