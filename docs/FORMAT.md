...
Version 1.0 of the reference implementation will mean this document is frozen at v3. The freeze rests on the vectors, the differential fuzz, and a second reader written from this document without reading the reference; both readers share an author, and no third-party reader exists at the freeze. After 1.0, a change to the rules is a new tag, and the reference reader keeps reading v3 alongside it. A third-party reader that later finds an outcome this document does not determine is answered the same way: the gap is closed under a new tag, and v3 records keep reading as they did.

Anyone may fork this format under the licence. A fork that changes any rule above uses its own namespace in the tag (`example.org/package.v1`, not `sealedrecord/package.v4`), so that no reader can mistake one format for the other and no record can claim conformance it does not have. The licence does not enforce this; it is a conformance rule. A record tagged `sealedrecord/package.v3` that does not follow this document is not a v3 record, whatever produced it, and the name `sealedrecord` is not licensed as a trademark (Apache-2.0, section 6). A fork that keeps the rules and only ships a different reader keeps the tag; conformance is what the vectors test, not who wrote the code.
